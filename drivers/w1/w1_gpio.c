 ...
 */
__STATIC_INLINE void w1_delay_index(struct w1_gpio_context *context,
	u8_t delay_idx)
{
	w1_delay(context->delays[delay_idx]);
}

__STATIC_INLINE void w1_gpio_set_dq(struct w1_gpio_context *context,
	int state)
{
	gpio_pin_write(context->gpio, context->dq_pin, state);
}

__STATIC_INLINE u32_t w1_gpio_get_dq(struct w1_gpio_context *context)
{
	u32_t state;
...
__STATIC_INLINE bool w1_gpio_read_bit(struct device *dev)
{
...
__STATIC_INLINE u8_t w1_gpio_read_byte(struct device *dev)
{
	u8_t byte = 0;
...
__STATIC_INLINE void w1_gpio_write_bit(struct device *dev, bool bit)
{
	struct w1_gpio_context *context = dev->driver_data;
...
__STATIC_INLINE void w1_gpio_write_byte(struct device *dev,
	const u8_t byte)
{
	u8_t curr_byte = byte;
...
__STATIC_INLINE bool w1_gpio_reset_bus(struct device *dev)
{
	struct w1_gpio_context *context = dev->driver_data;
...
__STATIC_INLINE void w1_gpio_write_block(struct device *dev,
...
__STATIC_INLINE void w1_gpio_read_block(struct device *dev, u8_t *block,
	u32_t length)
{
	/* loop for each byte of block */
...
__STATIC_INLINE int w1_gpio_wait_for(struct device *dev,
...
__STATIC_INLINE void w1_gpio_lock_bus(struct device *dev)
{
	struct w1_gpio_context *context = dev->driver_data;
...
__STATIC_INLINE void w1_gpio_unlock_bus(struct device *dev)
{
	struct w1_gpio_context *context = dev->driver_data;
...
__STATIC_INLINE int w1_gpio_init(struct device *dev)
{
	struct w1_gpio_context *context = dev->driver_data;
